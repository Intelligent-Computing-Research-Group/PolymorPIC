...
class StreamReaderCore(nXacts: Int, outFlits: Int, maxBytes: Int)
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val xactOnehot = PriorityEncoderOH(~xactBusy) // 0001
    ...
    io.out.bits.data.data := tl.d.bits.data   // data
    ...
    io.resp.valid := state === s_resp       // Finish
    io.resp.bits := true.B
    ...
    when (tl.a.fire) {
      ...
      when (sendlen === reqBytes) { // finish
        xactLast := (xactLast & ~xactOnehot) | Mux(sendpart, 0.U, xactOnehot)  // If it is the last one, set the bit at the assigned channel position in xactLast to 1.
        xactRightKeep(xactId) := rkeep      // If it is the last iteration, determine which specific bytes out of the eight are to be fetched.
        state := s_resp
      } .otherwise {
...
class StreamWriter(nXacts: Int, maxBytes: Int)
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val endAddr = baseAddr+length
    // val baseByteOff = baseAddr(byteAddrBits-1, 0)
    val baseByteOff = baseAddr(byteAddrBits-1, 0)
    val endByteOff = endAddr(byteAddrBits-1, 0)
    ...
    val sendTrail = (bytesToSend === endByteOff)&(endByteOff<=baseByteOff)&(offset>0.U)
    ...
    when (tl.a.fire) {
      ...
      when (bytesSent === bytesToSend) { 
        overhang:=0.U
        state := s_resp 
      }
    }

    dontTouch(bytesToSend)
    dontTouch(wmask)
