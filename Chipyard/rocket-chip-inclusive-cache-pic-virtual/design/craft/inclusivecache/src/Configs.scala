...
class WithInclusiveCache(
  ...
  case SubsystemBankedCoherenceKey => up(SubsystemBankedCoherenceKey, site).copy(coherenceManager = { context =>
    ...
    val mbus = context.tlBusWrapperLocationMap(MBUS)
    ...
    // extra tl nodes for polymorpic
    sbus.coupleFrom("stream-reader") { _ := l2.pic_dmaReaderNode }
    sbus.coupleFrom("stream-writer") { _ := l2.pic_dmaWriterNode }
    cbus.coupleFrom("tl_master_tlb_req") { _ := l2.pic_tlb_req_master_node }
    l2.pic_reveice_cmd_node := cbus.coupleTo("rocc_ctrl") { TLBuffer(1) := TLFragmenter(cbus) := _}
