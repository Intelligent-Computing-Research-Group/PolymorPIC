...
import_files -norecurse -fileset $source_fileset $files

if {[file exists "../../board/${vivado_board_name}/ethernet-${vivado_board_name}.v"]} {
  import_files -norecurse -fileset $source_fileset [file normalize "../../board/${vivado_board_name}/ethernet-${vivado_board_name}.v"]
}
...
# Some custom board has ddr pin settings
if {$vivado_board_name eq "ku060"} {
    set ddr_file_search_pattern "../../board/${vivado_board_name}/*_ddr.xdc"
    set ddr_files [glob $ddr_file_search_pattern]
    if {[llength $ddr_files] > 0} {
        puts "Find custom ddr pin settings."
        set files [concat $files $ddr_files]
    }
}
import_files -norecurse -fileset $constraint_fileset $files
...
###### Execute ethernet operation (if exists) ####
set tcl_file_path [file normalize "../../board/${vivado_board_name}/ethernet-${vivado_board_name}.tcl"]
if {[file exists $tcl_file_path]} {
    source $tcl_file_path
}

# Note: timing-constraints.tcl must be last
import_files -norecurse -fileset $constraint_fileset [file normalize ../../board/timing-constraints.tcl]
...
if { [get_files -quiet -of_objects $source_fileset [list "*/riscv_wrapper.v"]] == "" } {
  ...
  import_files -norecurse [file normalize vivado-${vivado_board_name}-riscv/${vivado_board_name}-riscv.srcs/sources_1/bd/riscv/hdl/riscv_wrapper.v ]
}
