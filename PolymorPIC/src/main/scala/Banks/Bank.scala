...
{
    val mat_num=sys_configs.mat_per_bank
...
object DescribedPolymorPIC {
  def apply[T <: Data](
    ...
    assert(size==PolymorPIC_Configs.sysCfg.bank_64b_num,s"Cache size not match!")
    assert(data.getWidth==64,s"Cache size not match!")
    val mem = Module(new Bank(sys_configs=PolymorPIC_Configs.sysCfg,core_configs=PolymorPIC_Configs.sysCfg.core_config))
