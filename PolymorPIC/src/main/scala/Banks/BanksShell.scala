...
{
  val matID=UInt(log2Ceil(sysCfg.total_valid_mats).W)
}

class BanksShell(sysCfg:Sys_Config,core_configs:PolymorPIC_Kernal_Config) extends Module 
{
    val bank_num=sysCfg.bank_num
    val mat_num_per_bank=sysCfg.mat_per_bank
    ...
    val io = IO(new Bundle {
        ...
        // Set up signals
        val set_up_addr=Input(UInt(log2Ceil(sysCfg.total_valid_mats).W))
        val set_up_io=new SetUpIO(sysCfg.core_config)

        // Load L
        val request_vec=Vec(sysCfg.total_valid_mats, Decoupled(new RequestAddress(core_configs)))
        val response_vec=Vec(sysCfg.total_valid_mats, Flipped(Decoupled(Bool())))
        val load_vec_en =Input(Bool())
        val load_vec_addr=Input(UInt(log2Ceil(sysCfg.total_valid_mats).W))
    ...
    banks_list.zipWithIndex.map{case(bank,bankID)=>
        ...
        bank.io.request_vec.zipWithIndex.map{case(mat_req_vec_io,matID)=>
            mat_req_vec_io  <> io.request_vec(matID*sysCfg.bank_num+bankID)
        }
        bank.io.response_vec.zipWithIndex.map{case(resp_vec_io,matID)=>
            resp_vec_io  <> io.response_vec(matID*sysCfg.bank_num+bankID)
        }
    }
