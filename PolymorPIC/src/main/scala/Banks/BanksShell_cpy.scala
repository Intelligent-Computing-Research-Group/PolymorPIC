...
// class BanksShell(sys_configs:Sys_Config,core_configs:PolymorPIC_Kernal_Config) extends Module 
// {
//     val bank_num=sys_configs.bank_num
//     val mat_num_per_bank=sys_configs.mat_per_bank
//     val mat_inner_offset=log2Ceil(core_configs.arrayNums*core_configs.wordlineNums)
//     val full_addrLen=log2Ceil(bank_num)+log2Ceil(mat_num_per_bank)+mat_inner_offset
...
//         // Set up signals
//         val set_up_addr=Input(UInt(log2Ceil(sys_configs.total_valid_mats).W))
//         val set_up_io=new SetUpIO(sys_configs.core_config)

//         // Load L
//         val request_vec=Vec(sys_configs.total_valid_mats, Decoupled(new RequestAddress(core_configs)))
//         val response_vec=Vec(sys_configs.total_valid_mats, Flipped(Decoupled(Bool())))
//         val load_vec_en =Input(Bool())
//         val load_vec_addr=Input(UInt(log2Ceil(sys_configs.total_valid_mats).W))
//         val vec_data=Input(UInt((core_configs.bitlineNums).W))

//         // Busy state of each mat
//         val query_matID=Input(UInt(log2Ceil(sys_configs.total_valid_mats).W))
//         val mat_busy=Output(Bool())
//     })


//     val banks_list=Seq.tabulate(bank_num) {
//         i =>
//         DescribedPolymorPIC(
//             name = s"cc_banks_$i",
//             desc = "Banked Store",
//             size = sys_configs.bank_64b_num,
//             data = UInt(64.W)
//         )
//     }

//     // Data IO
//     val access_bank_ID=sys_configs.get_bankID(io.cache_addr)
//     val cache_addr_bank_inner=sys_configs.get_fulladdr_sent_to_bank(io.cache_addr)
...
//     // setup
//     val set_up_selected_bank=sys_configs.get_bankID(io.set_up_addr)
//     val set_up_mat_offset=sys_configs.get_in_bank_matID(io.set_up_addr)
//     banks_list.zipWithIndex.map{case(bank,index)=>
...
//         set_up_io.if_signed        :=  io.set_up_io.if_signed
...
//     // Load L
//     val load_vec_bank=sys_configs.get_bankID(io.load_vec_addr)
//     val load_vec_in_bank_matID=sys_configs.get_in_bank_matID(io.load_vec_addr)
...
//             mat_req_vec_io  <> io.request_vec(matID*sys_configs.bank_num+bankID)
//         }
//         bank.io.response_vec.zipWithIndex.map{case(resp_vec_io,matID)=>
//             resp_vec_io  <> io.response_vec(matID*sys_configs.bank_num+bankID)
//         }
//     }

//     // query
//     val query_bankID=sys_configs.get_bankID(io.query_matID)
//     val query_in_bank_matID=sys_configs.get_in_bank_matID(io.query_matID)                          
