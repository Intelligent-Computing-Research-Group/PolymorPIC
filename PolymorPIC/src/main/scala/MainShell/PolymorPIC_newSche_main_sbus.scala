...
// Config file
case class PolymorPIC_newSche_Config()
case object PolymorPIC_newSche_cfg extends Field[Option[PolymorPIC_newSche_Config]](None)

// 这个模块模拟L2中的InclusiveCache那层, Bank部分在BankStore里面
class PolymorPIC_newSche_main_sbus(sysCfg:Sys_Config,core_configs:PolymorPIC_Kernal_Config)(implicit p: Parameters) extends LazyModule
{
    // DMA reader&writer
    ...
    // CMD_res
    val cmd_res=LazyModule(new CMD_resolver_new(sysCfg,core_configs)(p))
...
trait CanHaveExtraTileLinkConnectionNewSche { this: BaseSubsystem =>
  implicit val p: Parameters
  p(PolymorPIC_newSche_cfg).map { k =>
    val picDomain = sbus.generateSynchronousDomain
    picDomain{
      val polymorpic_sbus = LazyModule(new PolymorPIC_newSche_main_sbus(Sys_Config(),PolymorPIC_Kernal_Config())(p))
...
class WithPolymorPIC_newSche_on_sbus extends Config((site, here, up) => {
    case PolymorPIC_newSche_cfg => Some(PolymorPIC_newSche_Config())
})