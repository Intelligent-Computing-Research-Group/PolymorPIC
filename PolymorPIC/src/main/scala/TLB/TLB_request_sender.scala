...
class TLB_req_schedular(sys_configs:Sys_Config,core_configs:PolymorPIC_Kernal_Config)(implicit p: Parameters) extends LazyModule
{
    ...
    class TLB_req_schedular_impl extends LazyModuleImp(this)
    {
        val io = IO(new Bundle {
            val tlb_req=Vec(2, Flipped(Decoupled(new TLB_req())))
        ...
        val arbiter = Module(new RRArbiter(new TLB_req(), 2))
        for(i<-0 until 2)
        ...
        val mm_addr=sys_configs.query_tlb_MMIO
